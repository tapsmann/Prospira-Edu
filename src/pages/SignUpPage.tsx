...
export function SignUpPage() {
  const { user, loading, signUp, signInWithGoogle, signInWithGoogleRedirect } = useAuth()
  ...
  // Redirect flow lands back here; send signed-in users home.
  useEffect(() => {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (code === 'auth/popup-blocked') {
        // Popups are disabled here; fall back to a full-page redirect.
        try {
          await signInWithGoogleRedirect()
          return
        } catch (redirectErr: unknown) {
          const redirectCode = (redirectErr as { code?: string }).code || ''
          console.error('Google redirect sign-in failed:', redirectCode, redirectErr)
          setFirebaseError(getAuthErrorMessage(redirectCode))
          return
        }
      }
      setFirebaseError(getAuthErrorMessage(code))
      ...
      <div className="w-full max-w-md mx-auto" style={{ padding: '40px 20px' }}>
        <div className="text-center mb-8">
          ...
          <p style={{ fontFamily: B, color: '#6B6B6B', fontSize: '15px' }}>
            Start exploring pathways that fit who you are.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="mb-4">
            <label className="block text-sm font-medium mb-1.5" style={{ fontFamily: B, color: C.bodyText }}>Full Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Tafara Moyo"
              style={inputStyle(!!errors.name)}
              onFocus={(e) => e.currentTarget.style.borderColor = C.blue}
              onBlur={(e) => e.currentTarget.style.borderColor = errors.name ? '#DC2626' : C.border}
            />
            {errors.name && <p className="text-xs mt-1" style={{ color: '#DC2626', fontFamily: B }}>{errors.name}</p>}
          </div>
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium mb-1.5" style={{ fontFamily: B, color: C.bodyText }}>Confirm Password</label>
            <input
              type="password"
              ...
              onFocus={(e) => e.currentTarget.style.borderColor = C.blue}
              onBlur={(e) => e.currentTarget.style.borderColor = errors.confirmPassword ? '#DC2626' : C.border}
            />
            {errors.confirmPassword && <p className="text-xs mt-1" style={{ color: '#DC2626', fontFamily: B }}>{errors.confirmPassword}</p>}
          </div>
